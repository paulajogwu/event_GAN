...
function MakePayments({ eventId }: { eventId: string }) {
        ...
        <div className="sticky top-[4rem] px-4 py-5 2xl:px-6">
          ...
          <div className="mt-4 flex items-center justify-between gap-x-3 border-t px-0 py-2">
            ...
            <div className="text-base font-bold text-[#0F973D]">
              {fCurrency((data?.totalPrice ?? 0) * 0.2)}
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="w-full">
          ...
          {data && (
            <Elements
              ...
              options={{
                mode: "payment",
                amount: convertToSubcurrency((data?.totalPrice ?? 0) * 0.2),
                currency: "usd",
              }}
            >
              <CheckoutPage
                amount={(data?.totalPrice ?? 0) * 0.2}
                eventId={data?.id ?? ""}
              />
            </Elements>
          )}
